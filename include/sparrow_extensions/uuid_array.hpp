...
{
    /**
     * @brief UUID array implementation following Arrow canonical extension
     * specification.
     *
     * This class implements an Arrow-compatible array for storing UUID values
     * according to the Apache Arrow canonical extension specification for UUIDs.
    ...
    protected:
        ...
        {
            const size_t element_size = sparrow::num_bytes_for_fixed_sized_binary(proxy.format());
            SPARROW_ASSERT_TRUE(element_size == UUID_SIZE);
            // Add UUID extension metadata
            std::optional<sparrow::key_value_view> metadata = proxy.metadata();
            std::vector<sparrow::metadata_pair> extension_metadata = metadata.has_value()
    ...
    using uuid_array = sparrow::fixed_width_binary_array_impl<
...
{
    template <>
    struct get_data_type_from_array<sparrow_extensions::uuid_array>
    {
